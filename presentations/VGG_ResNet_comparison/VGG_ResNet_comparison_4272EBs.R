...
data <- read.csv("VGG_ResNet_4272EBs.csv",header=T,sep=",",stringsAsFactors=F, check.names=FALSE)
data <- melt(data,id.vars=c("Benchmakrs"))


data
data$a <- factor(data$Benchmakrs, levels = c("VGG16","ResNet18","All"))
data
pdf("VGG_ResNet_4272EBs.pdf",width=15,height=7)
